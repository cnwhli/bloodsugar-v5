...
- **微泰无数据修复**：短UUID的substring(4,8)崩溃连带丢微泰包，已加固；LT开头硅基发射器蓝牙名恢复匹配（上版误排除导致检测不到，已回退）
- **分钟级断流盯防**：蓝牙页开着时30秒查一次，90秒没新数打"没新数X分X秒"日志，3分钟报断链自动重扫（之前5分钟才报且没人调，7分钟空洞全程静默）
- **切后台断流修复**：之前后台isolate扫不到（MethodChannel没注册）+前台见后台在跑就跳过扫描，切后台两边都没在扫；现扫描永远归主isolate，后台只拿wake lock保活进程
